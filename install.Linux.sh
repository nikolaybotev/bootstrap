...
[ -f /usr/bin/yum ] && sudo yum install -y util-linux-user zsh git shadow
[ -f /usr/bin/apt-get ] && [ "$(whoami)" = "root" ] && apt-get update && apt-get install -y sudo
[ -f /usr/bin/apt-get ] && sudo apt-get update && sudo apt-get install -y zsh git shadow
[ -f /sbin/apk ] && [ "$(whoami)" = "root" ] && apk add --no-cache sudo
[ -f /sbin/apk ] && sudo apk add --no-cache zsh git shadow
